...
$(document).ready(function () {
    ...
    $('.story').waypoint(function () {
        ...
        var barheight = 20;
        var divwidth = $("#town").width();
        var myScaler2 = d3.scale.linear()
                .range([0, divwidth]);    
        var barChart = d3.select("#town svg")
                .attr("width", divwidth).attr("min-height", "100vh");     
            
        d3.json("density.json", function (cdata) {
            cdata.shift();
            cdata.splice(-2, 2);
            myScaler2.domain([0, d3.max(cdata, function(d){return type(d.people_total)})])

            var bar = barChart.selectAll("g")
                .data(cdata.filter(function (d) {
                    return d.site_id.substr(0, 3) == $(this.element).attr("id")
                }))
                .enter().append("g");
            
            bar.append("rect")
                .attr("width", function(d) {return myScaler2(d[1]);})
